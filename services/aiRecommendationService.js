...
exports.suggestAccessories = async (userId, message, opts = {}) => {
  ...
  try {
    ...
    if (Array.isArray(opts.excludeVariantIds) && opts.excludeVariantIds.length > 0) {
      where.push(`pv.id != ALL($${idx}::uuid[])`);
      params.push(opts.excludeVariantIds);
      idx++;
      console.debug('[suggestAccessories] excluding previously suggested variants', { 
        excludeCount: opts.excludeVariantIds.length 
      });
    }
...
exports.searchProducts = async (userId, message, opts = {}) => {
  ...
  try{
    ...
    //Loại bỏ variant đã gợi ý trước đó
    if(Array.isArray(opts.excludeVariantIds) && opts.excludeVariantIds.length > 0){
      where.push(`pv.id != ALL($${idx}::uuid[])`);
      params.push(opts.excludeVariantIds);
      idx++;
      console.debug('[searchProducts] excluding previously suggested variants', { 
        excludeCount: opts.excludeVariantIds.length 
      });
    }
